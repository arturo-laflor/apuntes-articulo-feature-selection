...
## Dataset
As a result of apply the questionnaire, a raw dataset $(m=342, n=47)$ was obtained, this dataset, have missing data, some columns are no significant in terms of variance, there exist data in a wrong format to analyze, among other features of the data without treatment. To obtain the dataset to the feature selection analysis, it was neccesary a pre-process of data, what included, put the data in right format, a cardinality analysis, a decision of delete incomplete rows vs the use of an algorithm of data imputation, and the use of an algorith applying the scale provided by the PSQI questionnaire to obtain the quality of sleep in a dicotomic variable. 
After this pre-process, the final dataset left conformed by 21 columns of predictive variables and one column for the target variable. The predictive variables are the factors of sleep hygiene provided by the SHI scale and the target variable is that resulting from the process of run the algorithm to compute the PSQI scale and evaluate it  through a simple if-else rule.  At the end, the dataset contains 21 ordinal-discrete variables in a 0-4 scale, where 0 is good and 4 is bad, and, one nominal variable that summarizes the quality of sleep in two ways, good qulaity of sleep and poor quality of sleep. 

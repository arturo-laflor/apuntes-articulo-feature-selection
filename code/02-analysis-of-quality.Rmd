...
The following tables show the report of qulaity in the dataset after apply deletion of missing values, exclusion of records containing outliers and imputation of missing values in variables that are closely related with the training of the model.
...
The final dataset to be used in the process of feature selection is a `r dim(QSSHICOMPLETE)[1] x dim(QSSHICOMPLETE)[2]` dataset containing 10 continuous features, 51 categorical features and one ID feature. The following table shows an outline of the dataset.
